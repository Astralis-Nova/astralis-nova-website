(() => {
  ...
  const ASSETS = {
    ...
    smile: "/planet-volcanic-real.svg",
    cry: "/astralis-mars.png",
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .astralis-hero-galaxy{position:absolute;z-index:1;right:-7%;top:-40%;width:clamp(330px,44vw,690px);aspect-ratio:1;pointer-events:none;opacity:.64;mix-blend-mode:screen;filter:drop-shadow(0 0 28px rgba(112,108,255,.24));transform-origin:50% 50%;animation:astralisGalaxyOrbit 138s linear infinite}
      .astralis-hero-galaxy img{display:block;width:100%;height:100%;object-fit:contain;animation:astralisGalaxyBreathe 11s ease-in-out infinite}
      .astralis-hero-galaxy::before{content:"";position:absolute;inset:17%;border:1px solid rgba(151,211,255,.13);border-radius:50%;box-shadow:0 0 35px rgba(115,137,255,.11),inset 0 0 35px rgba(255,118,213,.06);animation:astralisGalaxyCounterSpin 82s linear infinite reverse}
      .astralis-hero-galaxy::after{content:"";position:absolute;inset:31%;border-radius:50%;background:conic-gradient(from 30deg,transparent,rgba(126,218,255,.12),transparent 24%,rgba(255,132,219,.12),transparent 53%,rgba(255,228,141,.1),transparent);filter:blur(8px);animation:astralisGalaxyCounterSpin 58s linear infinite}
      ...
      .relic-icon.straggler-planet .relic-v5{--ring:#ff9857;--moon:#ffe0a8;--tilt:-33deg;--stretch:1.2;--orbit-speed:10.5s}
      .relic-icon.straggler-planet .relic-v6{--ring:#ff836f;--moon:#ffd2ad;--tilt:13deg;--stretch:1.3;--orbit-speed:13s}
      ...
      @keyframes astralisGalaxyBreathe{0%,100%{transform:scale(.985);opacity:.86}50%{transform:scale(1.025);opacity:1}}
      ...
      @media(max-width:800px){.astralis-hero-galaxy{right:-22%;top:-9%;width:460px;opacity:.42}}
      @media(max-width:520px){.astralis-hero-galaxy{right:-42%;top:0;width:390px;opacity:.32}}
  ...
  function addHeroGalaxy() {
    ...
    galaxy.innerHTML = '<img src="/astralis-hero-galaxy.svg" alt="">';
