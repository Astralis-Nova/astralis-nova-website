...
function glassSvg(type,color,suffix){
  ...
  // The drawing ends at y=164. Bottom alignment makes that drawn base—not the
  // SVG's surrounding box—the point planted on the square-center anchor.
  return `<svg viewBox="0 0 100 164" preserveAspectRatio="xMidYMax meet" aria-hidden="true"><defs><linearGradient id="${id}" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="${light}" stop-opacity=".92"/><stop offset=".38" stop-color="${mid}" stop-opacity=".32"/><stop offset=".65" stop-color="#fff" stop-opacity=".12"/><stop offset="1" stop-color="${deep}" stop-opacity=".5"/></linearGradient></defs>${base}${body}<path class="glass-highlight" stroke="${stroke}" d="M31 135 Q50 128 69 135"/></svg>`;
}
