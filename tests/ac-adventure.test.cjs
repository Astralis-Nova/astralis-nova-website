...
assert.deepEqual(cards, ['quests', 'directory', 'decal-plugins', 'new-player', 'conquest-quests', 'ac-media', 'ac-community']);
...
assert(html.includes("font-family:'Cinzel'"), 'Load the cinematic title font');
assert(css.includes('ac-title-shimmer'), 'Keep the title animation');
assert(css.includes('[data-ac-motion="on"]'), 'Title animation must follow the pause control');
